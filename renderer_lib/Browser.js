...
var _chromeTabsCss = require("./libs/chrome-tabs/chrome-tabs.css.js");

var _chromeTabsCss2 = _interopRequireDefault(_chromeTabsCss);
...
(0, _util.importStyle)(_chromeTabsCss2.default, "electron-puppeteer-chrome-tabs");
 ...
 */
class Browser extends _EventEmitter2.default {
  /**
   * Browser构造函数
   * @constructor Browser
   *
  ...
  constructor(browserManager, options) {
    ...
    this.isReady = false;
    this.isFront = false;
    this.id = (0, _util.uniqueId)("browser_");
  ...
  init() {
    ...
    return _asyncToGenerator(function* () {
      _this._handleWindowPage();
      _this.build();
      if (_this.options.createPage) {
        yield _this.newPage();
      }
      _this.isReady = true;
    })();
  ...
  build() {
    ...
    const pluginsHtml = this.options.plugins && this.options.plugins.length && this.options.plugins.some(item => item.visible == null || (typeof item.visible === "function" ? item.visible() : !!item.visible)) ? `<div class="chrome-tabs-button chrome-tabs-button-plugins" title="拓展功能">
  ...
  _initChromeTabs() {
    ...
    let chromeTabs = this._chromeTabs = new _chromeTabs2.default();
    ...
    if (this.doms.pluginsBtn) {
      ...
      this.doms.pluginsBtn.addEventListener("click", () => {
        const menu = new Menu();(this.options.plugins || []).forEach(item => {
          ...
          let menuItem = new MenuItem({
            ...
            checked: checked,
            icon: item.icon,
            visible: item.visible == null || (typeof item.visible === "function" ? item.visible() : !!item.visible),
            click: () => {
              let storeKey = storeKeyPrefix + item.key;
  ...
  isVisible() {
    return (0, _util.isElementInViewport)(this.element);
  }
  layoutTabs() {
   ...
   */
  _handleWindowPage() {
    remote.ipcMain.on(_ipc.BROWSER_WINDOW_PAGE_CONNECT, (evt, { openerWebContentsId, url, register }) => {
      let opener = this.getPageByWebContentsId(openerWebContentsId);
      if (opener) {
        let page = new _Page.WindowPage(this, _extends({}, this.options, {
          url
        }), evt.sender);
  ...
  _setCtrlIcons(ctrls) {
    if (Array.isArray(ctrls)) {
      ...
      ctrls.forEach(ctrl => {
        let el = (0, _util.parseStrToDOM)(`<div class="chrome-tabs-button chrome-tabs-button-custom"></div>`);
        if (ctrl.desc) {
        ...
        el.appendChild((0, _util.parseStrToDOM)(ctrl.icon));
        frag.appendChild(el);
      });
  ...
  _newPageWithoutReady(opener, url, options, handlerTab, isBack) {
    ...
    var page = new _Page2.default(this, {
      ...
      loadingTimeout: options.pageLoadingTimeout || this.options.pageLoadingTimeout,
  ...
  _handlePageTab(page) {
    ...
    let pageIcon = _images.faviconPng;
    let pageTitle;
    page.on("load-start", () => {
    ...
    page.on("favicon-updated", evt => {
      ...
      img.src = pageIcon;
      img.onerror = () => {
        pageIcon = _images.faviconPng;
        this._chromeTabs.updateTab(page.$tabEl, {
          favicon: _images.faviconPng,
          url: page.url()
        });
        img.onerror = null;
        img = null;
    ...
    page.on("new-window", evt => {
      if (!this.options.allowPopups || evt.disposition !== "new-window" && evt.disposition !== "foreground-tab") {
        evt.preventDefault();
        this._newPageWithoutReady(page, evt.url, {
          referrer: page.url()
        }, null, evt.disposition === "background-tab");
      }
    });
