...
class ChromeTabs {
  ...
  layoutTabs() {
    const tabContentWidths = this.tabContentWidths;
    let tabContentWidthsCount = 0;

    this.tabEls.forEach((tabEl, i) => {
      const contentWidth = tabContentWidths[i];
      const width = contentWidth + 2 * TAB_CONTENT_MARGIN;
      tabContentWidthsCount += contentWidth;
  ...
  addTab(tabProperties, { animate = true, background = false } = {}) {
    ...
    this.emit("tabAdd", _extends({ tabEl }, tabProperties));
  ...
  setTabCloseEventListener(tabEl) {
    tabEl.querySelector(".chrome-tab-close").addEventListener("click", () => this.removeTab(tabEl));
  }
  ...
  setCurrentTab(tabEl, trigger) {
    const activeTabEl = this.activeTabEl;
    if (activeTabEl === tabEl) return;
    if (activeTabEl) activeTabEl.removeAttribute("active");
    tabEl.setAttribute("active", "");
    this.emit("activeTabChange", { tabEl, trigger });
  }

  removeTab(tabEl, trigger) {
    ...
    this.emit("tabRemove", { tabEl, trigger });
    this.cleanUpPreviouslyDraggedTabs();
    this.layoutTabs();
    this.setupDraggabilly();
  }
