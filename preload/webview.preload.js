...
const isWindow = webContents.getType() === "window" || webContents.getType() === "browserView";
...
;(
  /* @module init */
  function(globalTask, parentTask, mainTask, ajaxInterceptorsFn, onHistoryNavigationFn, postMessageFn, ipcFn, jQueryProviderFn, utilFn) {
    var util = {};
    utilFn(util);
  ...
  function() {
    // 禁用beforeunload
    Object.defineProperty(window, 'onbeforeunload', { get(){}, set(){}})
  },
  /* @module parentTask */
  function(util, ipc, postMessage){
    var childFrames = new Map()
    postMessage.on(postMessage.CONST.WINDOW_OPEN_GOTO, function(evt, url) {
      location.href = url
    })
  ...
  /* @module mainTask */
  function(util, ipc, postMessage, ajaxInterceptors, onHistoryNavigation, jQueryProvider) {
    util.setIPC(ipc)
    util.setJQueryProvider(jQueryProvider)
    util.setAjaxInterceptors(ajaxInterceptors)

    var frameName
    var parentUUID
    var parentRoutingId
    var openerWebContentsId;
    const OPENER_SEP = ">|=$electron_puppeteer$=|<";
    if ((window.name || "").indexOf(OPENER_SEP) > -1) {
      [window.name, openerWebContentsId] = String(window.name).split(OPENER_SEP)
      openerWebContentsId = +openerWebContentsId
    }
    const isMainFrame = process.isMainFrame
    // 通过window.open打开的browserWindow
    const isWindowPage = isWindow && !!openerWebContentsId;
    const BROWSER_WINDOW_PAGE_CONNECT = 'electron-puppeteer_page_connect'
    const webviewParams = (() => {
      let webPreferences = webContents.getWebPreferences()
      return {
        allowPopups: webPreferences["electron-puppeteer-allow-popups"],
        browserId: webPreferences["electron-puppeteer-browser-id"],
        pageId: webPreferences["electron-puppeteer-page-id"],
      }
    })();
    ...
    function init() {
      // handle a target="_blank" click
      function isBlankLink(linkEl) {
        return linkEl.target && linkEl.hasAttribute("href") && !["_parent", "_top", "_self"].includes(linkEl.target) && !window.frames[linkEl.target]
      }
      function bindLinkElmClick(linkEl, force) {
        if (linkEl && (force || isBlankLink(linkEl)) && !linkEl._$$$_electron_puppeteer_link_click_binded) {
          linkEl._$$$_electron_puppeteer_link_click_binded = 1
          linkEl.addEventListener("click", function(evt) {
            // 也可能中途变更target, 所以需要再判断一次
            // 又因为只要target不是_开头的几个值，又不指向frame，就总是会新窗口打开
            if (isBlankLink(linkEl)) {
              setTimeout(() => {
      ...
      util.addEvent(document, "mouseover", function(evt) {
        let dom = evt.target;
        let linkEl;
        ...
        bindLinkElmClick(linkEl)
      })
      // rewrite document.createElement
      const originCreate = document.createElement;
      document.createElement = function() {
        let elm = originCreate.apply(this, arguments)
        if (elm.tagName === "A") {
          bindLinkElmClick(elm, true);
        }
        return elm
      }
      // rewrite window.open
      const originWindowOpen = window.open.bind(window);
      window.open = function(url, frameName, specs) {
        if (!url) {
          url = "about:blank"
        }
        switch(frameName) {
          case "_self":
            ...
            return window
          case "_parent":
            postMessage.send(window.parent, postMessage.CONST.WINDOW_OPEN_GOTO, url, frameName, specs)
            return window.parent
          case "_top":
            postMessage.send(window.top, postMessage.CONST.WINDOW_OPEN_GOTO, url, frameName, specs)
            return window.top
          default:
            let windowProxy;
            let parsedUrl = util.parseUrl(url);
            if (!webviewParams.allowPopups || (!specs && parsedUrl.host === location.host && parsedUrl.protocol === location.protocol.slice(0, -1))) {
              let proxyId = util.uniqueId("window_proxy_id");
              let locationProxyData = {
                href: util.absoluteURL(url),
                search: (parsedUrl.query ? "?" : "") + (parsedUrl.query || ""),
                hash: (parsedUrl.hash ? "?" : "") + (parsedUrl.hash || ""),
                pathname: parsedUrl.path || ""
              }
              let locationProxy = new Proxy(locationProxyData, {
                get(obj, key) {
                  if (key === "replace") {
                    return (url) => {
                      ipc.send("windowProxy.location", {
                        proxyId,
                        key: "href",
                        value: url
                      })
                ...
                set(obj, key, value) {
                  // 跨域则禁止
                  if (locationProxyData.host !== parsedUrl.host || locationProxyData.protocol !== parsedUrl.protocol) {
              ...
              let windowProxyData = {
                ...
                closed: false,
                close() {
                  ipc.send("windowProxy.close", {proxyId});
                  this.closed = true
                },
                focus() {
                  ipc.send("windowProxy.focus", {proxyId});
                },
                postMessage(message, targetOrigin) {
                  // todo 
                },
              }
              ipc.on("windowProxy.location.change", (param) => {
                if (param.proxyId === proxyId) {
                  parsedUrl = util.parseUrl(param.url);
                  Object.assign(locationProxyData, {
                    url: param.url,
                    search: (parsedUrl.query ? "?" : "") + (parsedUrl.query || ""),
                    hash: (parsedUrl.hash ? "?" : "") + (parsedUrl.hash || ""),
                    pathname: parsedUrl.path || ""
              ...
              ipc.on("windowProxy.closed.change", (param) => {
                if (param.proxyId === proxyId) {
                  windowProxyData.closed = true
                }
              });
              ipc.send("page.foreground-tab", {
                proxyId: proxyId,
                url: util.absoluteURL(url),
                referrer: location.href
              });
              windowProxy = new Proxy(windowProxyData, {
                get(obj, prop) {
                  return obj[prop]
                },
                set(obj, prop, value) {
              ...
              windowProxy = originWindowOpen.call(this, url, [frameName || "", webContentsId].join(OPENER_SEP), specs)
            }

            if (windowProxy && !windowProxy.document) {
      ...
      if (isWindowPage) {
        // frame.register需要合并发送
        ipcRenderer.send(BROWSER_WINDOW_PAGE_CONNECT, {
          openerWebContentsId: +openerWebContentsId,
          url: location.href,
          register: {
            UUID: UUID,
      ...
      util.onhashchange(() => {
        let parsedUrl = util.parseUrl(location.href);
        ipc.send(['frame.hashchange', 'frame.waitForNavigation.hashchange', 'frame.goto.hashchange'], {
          ...parsedUrl,
          UUID: UUID,
          name: frameName,
          url: location.href,
      ...
      onHistoryNavigation(function(payload) {
        ipc.send(['frame.historyNavigation', 'frame.waitForNavigation.historyNavigation', 'frame.goto.historyNavigation'], {
          UUID: UUID,
          name: frameName,
          url: payload.to || location.href,
          routingId: routingId,
          parentUUID: parentUUID,
          parentRoutingId: parentRoutingId,
          isMainFrame: isMainFrame
      ...
      util.addEvent(document, 'DOMContentLoaded', function() {
        function onCaptureEvent(evt) {
          ...
          ipc.send(["frame.event", "page.event"], {
            frame: frameName,
            type: evt.type,
            event: {
              type: evt.type,
        ...
        function catpureEvents(events) {
          for (let event of events) {
            util.addEvent(document.body, event, onCaptureEvent, false)
          }
        }
    ...
    var listenerMessages = {
      ...
      'frame.waitForFunction': function(param, evt) {
        ...
        return new Promise(function(resolve, reject) {
          ...
          var interval = setInterval(function() {
            ...
            try {
              var ret = util.evaluate(param.pageFunction, param.args, param.name || param.options && param.options.name);
            } catch(e) {
      ...
      'frame.waitForSelector': function(param, evt) {
        ...
        return new Promise(function(resolve, reject) {
          ...
          function check() {
            ...
            var visibleValid = !options || !options.visible || style.display != 'none' && style.visibility != 'hidden';
            var hiddenValid = !options || !options.hidden || style.display == 'none' || style.visibility == 'hidden';
          ...
          var timer = setInterval(function() {
            ...
            if (timeout != 0 && costTime >= timeout) {
              clearInterval(timer);
              reject('waitForSelector.timeout:' + param.selector);
            } else {
              check();
            }
          }, 50);
      ...
      'frame.evaluate': function(param, evt) {
        ...
        return util.evaluate(param.pageFunction, param.args, param.name);
      },
      ...
      'frame.addStyleTag': function(param, evt) {
        var id = param.id || util.uniqueId("style_")
      ...
      'elementHandle.$eval': function(param, evt) {
        ...
        var args = param.args || [];
        args.unshift(elm ? elm.querySelector(param.selector) : null);
  
        return util.evaluate(param.pageFunction, args, param.name);
      },
      'elementHandle.$$eval': function(param, evt) {
        if (!param || !param.pageFunction || !param.selector) {
          return Promise.reject('need pageFunction and selector');
        ...
        var args = param.args || [];
        args.unshift(elm ? elm.querySelectorAll(param.selector) : null);
  
        return util.evaluate(param.pageFunction, args, param.name);
      },
      'elementHandle.boundingBox': function(param, evt) {
        if (!param || !param.selector) {
          return Promise.reject('need selector');
        }
      ...
      'elementHandle.offsetTop': function(param, evt) {
        if (!param || !param.selector) {
          return Promise.reject('need selector');
        }
      ...
      'elementHandle.offsetLeft': function(param, evt) {
        if (!param || !param.selector) {
          return Promise.reject('need selector');
        }
      ...
      'elementHandle.show': function(param, evt) {
        if (!param || !param.selector) {
          return Promise.reject('need selector');
        }
        ...
        elm.style.display = options.display || "block"
        return true
      },
      'elementHandle.hide': function(param, evt) {
        if (!param || !param.selector) {
          return Promise.reject('need selector');
        }
      ...
      'elementHandle.textContent': function(param, evt) {
        if (!param || !param.selector) {
          return Promise.reject('need selector');
        }
      ...
      'elementHandle.press': function(param, evt) {
        ...
        return new Promise(function(resolve) {
          ...
          } else if (window.CompositionEvent) {
            elm.dispatchEvent(new CompositionEvent('compositionstart', {
              bubbles: true,
              composed: true,
              data: ""
            }));
          ...
          setTimeout(function() {
            ...
            if (param.keyCode == 8) {
              ...
              inputType = param.keyCode ? "insertText" : "insertCompositionText";
            ...
            if (!param.keyCode && window.CompositionEvent) {
              elm.dispatchEvent(new CompositionEvent('compositionend', {
                bubbles: true,
                composed: true,
                data: param.text
              }));
            ...
            // 在blur时触发change事件
            if (!elm._$$$_electron_puppeteer_blur_binded) {
              elm._$$$_electron_puppeteer_blur_binded = 1;
              elm.addEventListener('blur', function() {
                elm._$$$_electron_puppeteer_blur_binded = 0;
      ...
      'elementHandle.type': function(param, evt) {
        // todo
      },
      ...
      'page.ajaxInterceptors.add': function(interceptor) {
        return ajaxInterceptors.add(interceptor)
      },
      'page.ajaxInterceptors.remove': function(interceptorId) {
        return ajaxInterceptors.remove(interceptorId)
      }
    }
    
    Object.keys(listenerMessages).forEach(function(msg) {
      var listener = listenerMessages[msg]
      ipc.on(msg, listener, true)
    });
  },
  /* @module ajaxInterceptorsFn */
  function(util) {
    var interceptors = []
    var originOpen = window.XMLHttpRequest.prototype.__open__ = window.XMLHttpRequest.prototype.open
    var originSend = window.XMLHttpRequest.prototype.__send__ = window.XMLHttpRequest.prototype.send
    var originSetRequestHeader = window.XMLHttpRequest.prototype.__setRequestHeader__ = window.XMLHttpRequest.prototype.setRequestHeader
    ...
    Object.defineProperty(XMLHttpRequest.prototype, 'open', {
      ...
      value: function(method, url) {
        this.__ep_request_method = method
        this.__ep_request_url = url
        this.__ep_request_headers = {}
        var _onreadystatechange = this.onreadystatechange
        var _getResponseHeader = this.getResponseHeader
        var _getAllResponseHeaders = this.getAllResponseHeaders
        
        let parsedUrl = util.parseUrl(url)
        let search = new URLSearchParams(parsedUrl.query)
        let interceptor = interceptors.find(
          (item) =>
            (!item.method || item.method.toLowerCase() === method.toLowerCase()) 
            && (!item.protocol || (util.isArray(item.protocol) ? item.protocol.indexOf(parsedUrl.protocol) > -1 : item.protocol === parsedUrl.protocol)) 
            && (!item.host || (util.isArray(item.host) ? item.host.indexOf(parsedUrl.host) > -1 : item.host === parsedUrl.host)) 
            && (!item.path || (util.isArray(item.path) ? item.path.indexOf(parsedUrl.path) > -1 : item.path === parsedUrl.path)) 
            && (!item.query || (util.isArray(item.query) ? item.query.indexOf(parsedUrl.query) > -1 : item.query === parsedUrl.query))
            && (!item.search || (util.isObject(item.search) ? Object.keys(item.search).every(key => search.get(key) === item.search[key]) : util.isArray(item.search) ? item.search.every(f => search.get(f.key) === f.value) : false))
            && (!item.url || (util.isArray(item.url) ? item.url.indexOf(parsedUrl.url) > -1 : util.isFunctionString(item.url) ? util.evaluate(item.url, [parsedUrl, url], item.name) : util.parseUrl(item.url).url === parsedUrl.url))
        )
        
        if (interceptor) {
          this.__ep_interceptor = interceptor
          let interceptorRequest = interceptor.request
  
          if (interceptorRequest) {
            try {
              if (util.isString(interceptorRequest.url) || util.isFunction(interceptorRequest.url) ) {
                arguments[1] =
                  util.isFunctionString(interceptorRequest.url)
                    ? util.evaluate(interceptorRequest.url, [parsedUrl, interceptor], interceptor.name)
                    : interceptorRequest.url
              }
      
              if (util.isString(interceptorRequest.path) || util.isFunction(interceptorRequest.path)) {
                arguments[1] =
                  parsedUrl.protocol + "://" + parsedUrl.host +
                  (
                    util.isFunctionString(interceptorRequest.path)
                      ? util.evaluate(interceptorRequest.path, [parsedUrl, interceptor], interceptor.name)
                      : interceptorRequest.path
                  )
                  + parsedUrl.query
              }
      
              if (util.isString(interceptorRequest.query)  || util.isFunction(interceptorRequest.query)) {
                arguments[1] =
                  parsedUrl.protocol + "://" + parsedUrl.host + parsedUrl.path
                  (
                    util.isFunctionString(interceptorRequest.query)
                      ? util.evaluate(interceptorRequest.query, [parsedUrl, interceptor], interceptor.name)
                      : interceptorRequest.query
                  )
              }
            } catch(err) {
              try {
                interceptor && interceptor.onError && interceptor.onError(`interceptors send error, id: ${interceptor.id}, url: ${this.__ep_request_url}, error: ${err}`)
              } catch{}
            }
          }
          
          let interceptorResponse = interceptor && interceptor.response
          if (interceptorResponse) {
            try {
              this.onreadystatechange = (evt) => {
                if (this.readyState === this.HEADERS_RECEIVED && interceptorResponse.headers) {
                  try {
                    let parsedUrl = util.parseUrl(this.__ep_request_url)
                    if (util.isFunctionString(interceptorResponse.headers)) {
                      interceptorResponse.formatHeaders = util.evaluate(interceptorResponse.headers, [this.getAllResponseHeaders(), parsedUrl, interceptor], interceptor.name)
                    } else {
                      interceptorResponse.formatHeaders = Object.assign({}, interceptorResponse.headers || {})
                    }
  
                    Object.defineProperty(this, 'getResponseHeader', {
                      value: function(key) {
                        return interceptorResponse.formatHeaders[key] || _getResponseHeader.call(this, key)
                      }
                    })
  
                    Object.defineProperty(this, 'getAllResponseHeaders', {
                      value: function() {
                        return Object.assign({}, _getAllResponseHeaders.call(this), interceptorResponse.formatHeaders)
                      }
                    })
                  } catch (err) {
                    try {
                      interceptor && interceptor.onError && interceptor.onError(`interceptors readyState HEADERS_RECEIVED error, id: ${interceptor.id}, url: ${this.__ep_request_url}, error: ${err}`)
                    } catch{}
                  }
                }
                ...
                if (this.readyState === this.DONE && this.status < 400) {
                  try {
                    if (util.isString(interceptorResponse.body) || util.isFunction(interceptorResponse.body)) {
                      Object.defineProperty(this, 'responseText', {
                        value: util.isFunctionString(interceptorResponse.body)
                          ? util.evaluate(interceptorResponse.body, [].concat(interceptorResponse.bodyArgs || []).concat([{
                            url: this.__ep_response_url,
                            path: util.parseUrl(this.__ep_response_url).path,
                            body: this.responseText,
                            headers: this.getAllResponseHeaders()
                          }, {
                            url: this.__ep_request_url,
                            path: util.parseUrl(this.__ep_request_url).path,
                            method: this.__ep_request_method,
                            body: this.__ep_request_body,
                            headers: this.__ep_request_headers
                          }, interceptor], interceptor.name))
                          : interceptorResponse.body
                      })
                    }
                  } catch (err) {
                    try {
                      interceptor && interceptor.onError && interceptor.onError(`interceptors readyState DONE error, id: ${interceptor.id}, url: ${this.__ep_request_url}, error: ${err}`)
                    } catch{}
                  }
                }
                
                if (_onreadystatechange) {
                  _onreadystatechange.call(this, evt);
                }
              }
              ...
              try {interceptor.onError && interceptor.onError(`interceptors onreadystatechange error, id: ${interceptor.id}, url: ${this.__ep_request_url}, error: ${err}`)}catch{}
            }
          }
        }
        
        this.__ep_response_url = arguments[1]
    ...
    Object.defineProperty(XMLHttpRequest.prototype, 'setRequestHeader', {
      ...
      value: function(header, value) {
        let interceptor = this.__ep_interceptor
        let interceptorRequest = interceptor && interceptor.request
        if (interceptorRequest) {
          try {
            let parsedUrl = util.parseUrl(this.__ep_request_url)
            interceptorRequest.formatHeaders = interceptorRequest.formatHeaders || (
              util.isFunctionString(interceptorRequest.headers) 
              ? util.evaluate(interceptorRequest.headers, [parsedUrl, interceptor], interceptor.name)
              : Object.assign({}, interceptorRequest.headers || {})
            )
  
            if (header in interceptorRequest.formatHeaders) {
              arguments[1] = interceptorRequest.formatHeaders
            }
          } catch(err) {
            try {
              interceptor.onError && interceptor.onError(`interceptors setRequestHeader error, id: ${interceptor.id}, url: ${this.__ep_request_url}, error: ${err}`)
            } catch{}
          }
        }
        
        this.__ep_request_headers[arguments[0]] = arguments[1]
    ...
    Object.defineProperty(XMLHttpRequest.prototype, 'send', {
      ...
      value: function(data) {
        ...
        if (interceptorRequest) {
          try {
            if (util.isObject(interceptorRequest.formatHeaders)) {
              for (let key in interceptorRequest.formatHeaders) {
                if (!this.__ep_request_headers.hasOwnProperty(key)) {
                  this.__ep_request_headers[key] = interceptorRequest.formatHeaders[key]
                  originSetRequestHeader.call(this, key, interceptorRequest.formatHeaders[key])
                }
              }
            }
      
            if (util.isString(interceptorRequest.body) || util.isFunction(interceptorRequest.body)) {
              arguments[0] = util.isFunctionString(interceptorRequest.body)
              ? util.evaluate(interceptorRequest.body, [].concat(interceptorRequest.bodyArgs || []).concat([{
                url: this.__ep_request_url,
                path: util.parseUrl(this.__ep_request_url).path,
                method: this.__ep_request_method,
                body: data,
        ...
        this.__ep_request_body = arguments[0]
    ...
    return {
      add: function(interceptor) {
        interceptor.id = util.uniqueId("interceptor_")
        interceptors.push(interceptor)
        return interceptor.id
      },
      remove: function(interceptorId ) {
        interceptors = interceptors.filter(item => item.id !== interceptorId)
        return true
      }
    }
  },
  /* @module onHistoryNavigationFn */
  function(util) {
    ...
    function instrumentHistory() {
      ...
      const captureUrlChange = (from, to) => {
        const parsedLoc = util.parseUrl(location.href);
        const parsedTo = util.parseUrl(to);
        let parsedFrom = util.parseUrl(from);
        ...
        callbacks.forEach(cb => setTimeout(() => {
          cb({from, to})
        }, 0))
      };
    ...
    instrumentHistory()
    return function onHistoryNavigation(cb) {
      callbacks.push(cb)
    }
  },
  ...
  function(util) {
    ...
    return {
      ...
      CONST: {
        ...
        // window.open跳转
        WINDOW_OPEN_GOTO: "window.open.goto",
      }
    }
  },
  /* @module ipcFn */
  function(util, exports) {
    ...
    const RECOVER_NAME = 'electron-puppeteer_recover'
    // default options
    let _options = {
      timeout: 1e4
    }
    ...
    const sendToHost = (...args) => {
      if (isWebview) {
        return ipcRenderer.sendToHost(...args)
      }
   ...
   */
    ...
    ipcRenderer.on(RECEIVE_NAME, function(originalEvent, evt) {
      ...
      for (let i = 0; i < _listeners.length; i++) {
        ...
        if ((evt.UUID === '*' || UUID === evt.UUID) && item.name === evt.name && !!item.isAck === !!evt.isAck) {
          ...
          results.push(item.listener.call(this, evt.payload, evt, evt.error))
          break
        }
      }
      ...
      if (!evt.isAck && evt.ack) {
        Promise.race(results.slice(0, 1)).then(result => {
          ...
          isDevelopment && ipcLog && console.error('%cipc reply error', 'font-weight:bold;color:#f00', `name: ${evt.name},`, `err: ${JSON.stringify(err)}`)
          sendToHost(SEND_NAME, {
            UUID: UUID,
            routingId: routingId,
            name: evt.ack,
    ...
    exports.send = function(name, payload, timeout) {
      ...
      return new Promise((resolve, reject) => {
        ...
        if (err === 'preload.ipc.timeout') {
          ...
          console.error('ipc reply error:', err)
        }
        return {error: err}
      })
    }
     ...
     * 
     */
    exports.on = function(name, listener, unique) {
      if (unique) {
        this.off(name)
      }
      _listeners.push({
  ...
  /* @module utilFn */
  function(exports) {
    var ipc, jQueryProvider, ajaxInterceptors;
    ...
    function evaluate(expression, args, key) {
      try {
        let debugLine = key ? `\n //# sourceURL=dynamicScript.${key}.js;` : ''
        var method = isFunction(expression) ? expression : globalEval("(" + expression + `);${debugLine}`);
      } catch(err) {
      ...
      return method.apply(
        ...
        args.map(function (item) {
          ...
          if (item === "ajaxInterceptors") {
            return ajaxInterceptors
          }
    ...
    function isFunctionString(a) {
      return (
        isFunction(a) ||
          "string" == typeof a &&
          ((a.indexOf("function") > -1 &&
            /^\s*function\s+\S+\s*\([^)]+\)\s*{/.test(a)) ||
            (a.indexOf("=>") > -1 && /^\s*(\([^)]+\)|\S+)\s*=>/.test(a)))
      )
    }    
    ...
    function jsonp(url, param, callback) {
      ...
      var id = uniqueId('__ep_jsonp_script_id');
      var callbackName =  uniqueId('__ep_jsonp_cb');
    ...
    function parseUrl(url){
      ...
      let protocol = match[2] || ""
      let host = match[4] || "" 
      const query = match[6] || '';
      const fragment = match[8] || '';
      const relative = match[5] + query + fragment
      ...
      return {
        ...
        query: query,
        hash: match[9] || "",
        url: protocol !== "about" ? protocol + "://" + host + relative : url
      }
    }

    function onhashchange(callback, triggerOnStart) {
      if (triggerOnStart) {
        callback()
      }
    ...
    function deleteAllCookies(path = "/", domain = "") {
      ...
      for (var i = 0; i < cookies.length; i++) {
        ...
        document.cookie = `${name}=; expires=${new Date(0).toGMTString()}; path=${path}; domain=${domain}`;
      }
    }
    ...
    Object.assign(exports, {
      ...
      setIPC: function(_ipc) {
        ipc = _ipc
      },
      setJQueryProvider: function(fn) {
        jQueryProvider = fn
      },
      setAjaxInterceptors: function(_ajaxInterceptors) {
        ajaxInterceptors = _ajaxInterceptors
      }
    })
    window._electron_puppeteer_inject_util_ = exports;
  }
  )
)